...
export function AppSidebar({ items, locale, user }: { items: NavItem[]; locale: string; user: UserCardProps }) {
  ...
  return (
    <Sidebar collapsible="icon" side={locale === "he" ? "right" : "left"}>
      <SidebarHeader className="h-14 justify-center border-b border-sidebar-border px-3"><Link href="/app"><Logo collapsible tone="ink" /></Link></SidebarHeader>
      <SidebarContent>
        {groups.map((group, i) => (
                  ...
                  return (
                    <SidebarMenuItem key={it.href}>
                      <SidebarMenuButton asChild isActive={active} tooltip={it.label}>
                        <Link href={it.href}><Icon />{it.label}</Link>
                      </SidebarMenuButton>
                    </SidebarMenuItem>
                  );
                })}
